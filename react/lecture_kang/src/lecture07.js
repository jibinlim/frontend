...
export default function App() {
  ...
  return (
    ...
    <button onClick={() => setNation('중국')}>중식</button>
    {nation === '' ? 
      <ul>
        {foodList.map((item, index) => <li key={index}>{item.food}</li>)}
      </ul> 
      :
      <ul>
        {foodList.filter((item) => item.nation === nation).map((item, index) => <li key={index}>{item.food}</li>)}
      </ul> 
    } 
    </>
  );
